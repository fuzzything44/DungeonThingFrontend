...
const Quest: React.FC<QuestProps> = (props) => {
    ...
    return <div style={{ marginBottom: "0.7em" }}>
        <button onClick={toggleExpand}>
            ...
            {props.name}{" "}
            ({Math.min(props.progress, props.max_progress)}/{props.max_progress})
        </button>
        {complete && !props.claimed ?
            <button
                onClick={async () => {
